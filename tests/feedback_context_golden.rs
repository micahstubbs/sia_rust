...
fn test_success_single_with_results() {
    ...
    std::fs::write(gen_dir.join("agent_execution.json"), json!([{"role": "user", "content": "solve it"}]).to_string())
        .unwrap();
    std::fs::write(gen_dir.join("results.json"), json!({"accuracy": 0.9, "correct": 9, "total": 10}).to_string())
        .unwrap();
    let stdout_log = gen_dir.join("target_agent_stdout.log").to_string_lossy().into_owned();
...
fn test_failure_single_no_results() {
    ...
    let gen_dir = d.path().join("gen_1");
    std::fs::create_dir(&gen_dir).unwrap();
    std::fs::write(gen_dir.join("agent_execution.json"), json!([{"role": "user", "content": "attempt"}]).to_string())
        .unwrap();
    let stdout_log = gen_dir.join("target_agent_stdout.log").to_string_lossy().into_owned();
...
fn test_success_multi_with_results() {
    ...
    for i in 0..2 {
        std::fs::write(exec_dir.join(format!("execution_q{i}.json")), json!([{"role": "user", "content": format!("q{i}")}]).to_string())
            .unwrap();
    }
    std::fs::write(gen_dir.join("results.json"), json!({"accuracy": 0.8}).to_string()).unwrap();
    let stdout_log = gen_dir.join("target_agent_stdout.log").to_string_lossy().into_owned();
